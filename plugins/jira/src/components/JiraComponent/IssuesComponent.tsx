            ...
            <TableRow>
              <TableCell>
                  ...
                  >
                    ...
                    {users.map(user => (
                      <MenuItem
                        value={user.key}
                        key={user.key}
                      >{`${user.displayName} ${user.emailAddress}`}</MenuItem>
                    ))}
                  </Select>
                </FormControl>
                <FormControl variant="filled" margin="dense">
                ...
                <Button
                  ...
                  onClick={() => {
                    dispatch(
                      allActions.Actions.setIssuesSearch({
                        name: name.trim(),
                        status: status.trim(),
                      }),
                    );
...
const IssuesComponent: FC<{ projectKey: string }> = ({ projectKey }) => {
  ...
  const { value, loading, error } = useAsync(async (): Promise<IssuesProps> => {
    ...
    const issues = await fetch(
      `${url}/issues/${projectKey}?index=${index}${name}${status}`,
      options,
    );
    const issuesData = await issues.json();

    const users = await fetch(`${url}/users`, options);
    const usersData = await users.json();

    const statuses = await fetch(`${url}/statuses`, options);
